...
class getConsistencyMtx(Manager):
    ...
    def import_results(self):
        csv_path="/home/hayashide/kazu_ws/master_thesis/master_thesis_modules/scripts/AHP/questionare_answer.csv"
        data=pd.read_csv(csv_path)
        items=[40000010,40000011,40000012,40000013,40000014,40000015,40000016]
        items_ja=["立つ","ブレーキ解除","車椅子移動","姿勢崩す","挙手","咳き込む","顔を触る"]
        columns=[]
        for i,item1 in enumerate(items):
            if i!=len(items):
                for j,item2 in enumerate(items[i+1:]):
                    columns.append([item1,item2])
        
        weight_result_df=pd.DataFrame(columns=list(data.keys()))

        for staff in data.keys():
            A,eigvals,eigvecs,max_eigval,weights,CI=self.get_AHP_weight(criteria=items,comparison_answer=data[staff])
            weight_result_df[staff]=list(weights)+[CI]
        
        weight_result_df.to_csv(os.path.split(csv_path)[0]+"/results.csv",index=False)
