...
患者見守り支援は、転倒・離床・姿勢・活動の検知を中心に広く研究されてきた。ベッド周辺など局所的な環境を対象としたリスク評価に加え、電子カルテ・バイタル等を用いて病棟全体の患者状態を継続的に評価する方法も提案されている \cite{Mubashir2013SurveyFalls,Gutierrez2021VisionFallReview,Isomoto2018FallRisk,Rothman2013ContinuousCondition,Escobar2020Deterioration}。詳細はSection 2で述べる。

本研究では、こうした見守り支援を食堂・デイルームといった**病棟共有空間における複数患者の見守り**へ適用する。この環境には、危険度と見守り優先順位を評価するうえで重要な2つの特徴がある。

第一に、見守り優先順位は患者本人の状態・行動だけでなく、患者を取り巻く状況に依存する。Figure 1に示すように、同じ立ち上がり動作でも、スタッフが患者を直接介助している場合と、近くのスタッフが別業務に従事している場合では、見守り優先順位が異なり得る。本研究では、周囲物体、施設構造、スタッフと各患者の距離・方向、見守り状況など、患者本人以外の空間的状況を**spatial context（空間的文脈）**と定義する。また、患者の属性や動作といった状況を、空間的文脈と対比して患者文脈と定義する。
...
空間的文脈を用いたリスク評価と、専門家の判断をモデルへ反映する研究はそれぞれ存在する \cite{Koshmak2014ContextFallRisk,Boulet2020ExpertWeights,TenBroeke2021BAIT}。一方、病棟共有空間において、**空間的文脈とスタッフごとの判断知識を同時に扱い、複数患者の時系列危険度と見守り優先順位を評価する研究は、確認した範囲で限定的である**。空間的文脈を導入すると扱う特徴量が増え、患者行動と周囲状況の間には条件依存的な相互作用も生じ得る。そのため、すべての特徴量の組み合わせについてスタッフの判断を直接取得することは現実的ではない。
...
評価はRQ1とRQ2に沿って構成する。RQ1では、シミュレーションにより空間的文脈の有無だけを変化させ、その影響を確認する。RQ2では、統制したシミュレーションと病棟共有空間で取得した実データの両方を用い、スタッフごとの判断知識による出力差を確認する。
...
患者文脈のみを用いた条件では、空間的文脈の違いが出力に反映されず、各患者の立ち上がりに対して同様の危険度変化が生じた。一方、空間的文脈を含む提案手法では、「スタッフによる見守りあり・車椅子あり・点滴なし」、「見守りなし・車椅子あり・点滴なし」、「見守りなし・車椅子あり・点滴あり」の順に危険度が高くなった。
...
統制したシミュレーションシナリオに複数のスタッフ別モデルを適用し、スタッフごとの判断知識が見守り優先順位へ与える影響を確認する。判断傾向の異なるモデルを比較し、患者文脈と空間的文脈の重視度、および周囲物体とスタッフの見守り状況の重視度が、出力へどのように反映されるかを評価する。

**Scenario 1:** 4名の患者を2×2に配置し、スタッフを正面から4名を見渡せる位置に配置する。奥側の2名は手前側の2名よりスタッフから遠いため、スタッフとの距離に関する空間的文脈が異なる。ここで左側の2名に立ち上がり動作を与え、患者文脈と空間的文脈のどちらを重視するかが異なるスタッフ別モデル間で、見守り優先順位を比較する。
