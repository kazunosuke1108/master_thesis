...
* 病棟共有空間では、スタッフが複数患者の状態や行動を並行して把握し、対応対象を選択する必要がある。
* 患者見守り研究では、転倒、離床、姿勢、活動等の検知が広く研究されている \cite{Mubashir2013SurveyFalls,Gutierrez2021VisionFallReview,Balaguera2017MedicalIoT}。
* 一方、見守り上の危険性は患者本人の状態だけでなく、周囲の物体、施設構造、スタッフとの位置関係によっても変化する。病院内の空間的位置を用いた転倒リスク推定や、スマート環境におけるcontext-aware risk assessmentにも先行例がある \cite{Isomoto2018FallRisk,Koshmak2014ContextFallRisk}。
* また、同じ状況に対してどの因子を重視するかはスタッフ間で異なり得る。医療意思決定支援では、専門家の因子重みや条件間の関係をモデル化する研究も行われている \cite{Boulet2020ExpertWeights,TenBroeke2021BAIT}。
* 本研究では、病棟共有空間における患者文脈と空間的文脈を階層的に統合し、スタッフごとの判断知識を明示的なパラメータとして反映する危険度評価手法を提案する。
* 評価では、空間的文脈の有無、およびスタッフ別パラメータの違いが、危険度時系列と患者間順位へ与える影響を分析する。
...
専門家知識を意思決定モデルへ反映する方法として、医師が各臨床因子へ与える重要度を数値化する方法 \cite{Boulet2020ExpertWeights} や、仮想症例への選択から因子重みと条件間相互作用を推定する方法 \cite{TenBroeke2021BAIT} が提案されている。AHPとFuzzy推論も、多基準意思決定や医療リスク評価に広く用いられている \cite{Saaty1990AHP,Zadeh1965FuzzySets,Mamdani1975LinguisticControl,Uzoka2011FuzzyAHPClinical,Chang2022FuzzyAHPRisk}。
...
特徴量は性質の近い要素ごとに集約し、上位層で異なるカテゴリのリスクを統合する。
...
`Bさんが車椅子に乗っている状態で立ち上がったため、危険度が上昇しました。`

通知生成は、危険度評価結果の利用例として扱う。
...
### 4.2.1 Simulation data for RQ1

患者本人の行動条件を揃え、以下の空間的文脈を変化させた特徴量時系列を生成する。
...
### 4.2.2 Real-world data for RQ2

病棟共有空間で取得した実データに対して、スタッフごとのアンケート回答に基づくモデルを適用する。同一入力に異なるスタッフモデルを適用することで、パラメータ差による出力差を比較する。

必要に応じて、全回答者の平均パラメータを用いたAverage modelを補助基準とする。
...
* 空間的文脈変化前後の危険度差。
* 最大危険度・危険度変動幅。
* Patient context onlyとSpatial context includedで順位が異なる時間割合。
* 患者間の順位逆転。
...
* 最大危険度・危険度変動幅・ピーク時刻。
* スタッフモデル間の患者順位差。
* アンケートから得た判断傾向と波形特徴量の関係。

AHP回答についてはCIまたはCRを算出し、回答の整合性を確認する。

## 4.4 Evaluation 1: Effect of Spatial Context

空間的文脈の異なる複数患者が相次いで危険行動をとるシナリオを用いる。

Patient context onlyとSpatial context includedを比較し、

1. 同一または類似した患者行動に対して危険度波形がどのように変化するか。
2. 空間的文脈によって患者間順位が入れ替わるか。

を分析する。

主な結果図として、危険度時系列と患者順位の推移を示す。

## 4.5 Evaluation 2: Reflection of Staff-specific Knowledge

### 4.5.1 Case study

判断傾向が対照的な2名を選定し、同一の実データへそれぞれのモデルを適用する。
