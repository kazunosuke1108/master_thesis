...
if __name__=="__main__":
    ...
    for staff_name_ahp in staff_names:
        for staff_name_fuzzy in staff_names:
            ...
            trial_name=f"20251211_MasterThesisData_{staff_name_ahp[0]}{staff_name_fuzzy[1]}"
            data_dicts=Manager().load_picklelog("/media/hayashide/MasterThesis/20251211_MasterThesisData/data_dicts.pickle")
            for patient, data in data_dicts.items():
                columns=[]
                for k in data.columns:
                    try:
                        columns.append(int(k))
                    except ValueError:
                        columns.append(k)
                data_dicts[patient].columns=columns
