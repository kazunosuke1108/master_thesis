...
source_dir_path="C:/Users/hyper/kazu_ws/master_thesis/master_thesis_modules/scripts/exp_analysis/next_action/sources"

# 対応の急ぎ度に関するランキングデータ
csv_path="C:/Users/hyper/kazu_ws/master_thesis/master_thesis_modules/scripts/exp_analysis/next_action/hurry_rank.csv"
...
for case_name in case_list:
    # データの抽出
    ranks=data[sorted([k for k in data.keys() if case_name in k])].values.astype(float)
    # Friedman検定の実行
...
for case_name in case_list:
    ...
    for comparison_pair in comparison_pair_list:
        try:
            ...
            result_dict["wilcoxon"][case_name][comparison_pair[0]]={}
        ranks_0=data[comparison_pair[0]].values.astype(float)
        ranks_1=data[comparison_pair[1]].values.astype(float)
        # Wilcoxon符号付き順位検定
