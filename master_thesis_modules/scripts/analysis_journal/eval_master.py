...
class RealtimeEvaluator(Manager,GraphManager):
    def __init__(self,trial_name,strage,attempt_name):
        ...
        self.tf_draw_name_on_bbox=True
    ...
    def load_data(self,iteration_idx=np.nan):
        def delate_nan_patient(json_data):
            # 有効なIDを取得（activeBinaryが1のIDのみ残す）
            valid_ids = {key.split("_")[0] for key, value in json_data.items() if key.endswith("_x") and (not np.isnan(value))}

            # valid_ids に含まれるIDのキーのみを残す
            filtered_data = {key: value for key, value in json_data.items() if key.split("_")[0] in valid_ids}

            return filtered_data

        if np.isnan(iteration_idx):
            raise NotImplementedError("リアルタイム処理はサポートされていません")
        ...
        while not all_files_found:
            try:
                ...
                self.json_latest_data=self.df_after_reid.iloc[iteration_idx-1].to_dict()
                self.json_previous_data=self.df_after_reid.iloc[iteration_idx-2].to_dict()
                self.json_latest_data=delate_nan_patient(json_data=self.json_latest_data)
                self.json_previous_data=delate_nan_patient(json_data=self.json_previous_data)
                # 患者情報
                ...
                elp_img_paths=sorted(glob(f"{self.data_dir_dict['mobilesensing_dir_path']}/jpg/elp/{self.left_right}/*.jpg"))
    ...
    def evaluate_main(self,iteration_idx=np.nan):
        if np.isnan(iteration_idx):
            
            raise NotImplementedError("Realtimeはサポートされていません")
        try:
            ...
            json_latest_path,json_latest_data=self.load_data(iteration_idx)
    ...
    def evaluate_main_offline(self):
        try:
            for iteration_idx in range(2,len(self.df_after_reid)):
                self.evaluate_main(iteration_idx=iteration_idx)
                if iteration_idx%(5*60)==0:
                    self.save()
    ...
    def save(self):
        ...
        self.df_eval.to_csv(self.data_dir_dict["mobilesensing_dir_path"]+f"/csv/df_eval_{self.attempt_name}.csv",index=False)
        self.df_eval.to_csv(self.data_dir_dict["mobilesensing_dir_path"]+f"/csv/df_eval.csv",index=False)
        self.df_post.sort_index(axis=1,inplace=True)
        self.df_post.to_csv(self.data_dir_dict["mobilesensing_dir_path"]+f"/csv/df_post_{self.attempt_name}.csv",index=False)
        self.df_post.to_csv(self.data_dir_dict["mobilesensing_dir_path"]+f"/csv/df_post.csv",index=False)
    
        save_notify_history=self.notify_history.sort_index(axis=1)
        save_notify_history.to_csv(self.data_dir_dict["mobilesensing_dir_path"]+f"/csv/notify_history_{self.attempt_name}.csv",index=False)
        save_notify_history.to_csv(self.data_dir_dict["mobilesensing_dir_path"]+f"/csv/notify_history.csv",index=False)
...
if __name__=="__main__":
    ...
    cls=RealtimeEvaluator(trial_name=trial_name,strage=strage,attempt_name=attempt_name)
    print("RealtimeEvaluator has woken up")
    cls_zokusei=PreprocessZokusei()
    print("PreprocessBlip has woken up")
    cls_yolo=PreprocessYolo()
