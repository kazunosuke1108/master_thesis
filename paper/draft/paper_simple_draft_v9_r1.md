...
患者見守り支援は、転倒・離床・姿勢・活動の検知を中心に広く研究されてきた。特定の空間・患者に絞って見守る、ベッド周辺の位置情報を用いたリスク評価や、逆に病棟全体を長期にわたり見守る、電子カルテ・バイタル等に基づく継続的な患者リスク評価も提案されている \cite{Mubashir2013SurveyFalls,Gutierrez2021VisionFallReview,Isomoto2018FallRisk,Rothman2013ContinuousCondition,Escobar2020Deterioration}。詳細はSection 2で述べる。
...
空間的文脈を用いたリスク評価と、専門家の判断をモデルへ反映する研究はそれぞれ存在する \cite{Koshmak2014ContextFallRisk,Boulet2020ExpertWeights,TenBroeke2021BAIT}。一方、病棟共有空間において、**空間的文脈とスタッフごとの判断知識を同時に扱い、複数患者の時系列危険度と見守り優先順位を評価する研究は、確認した範囲で限定的である**。しかし、両要求を満たすことは容易ではない。なぜなら、特徴量の多いことに加え、特徴量間の相互作用についても判断知識をスタッフから取得することが必要であり、これらをスタッフから網羅的に取得することは困難だからであると考えられる。
...
これらは空間的文脈を扱ううえで重要な先行研究であり、本研究でも、患者文脈と空間的文脈を統合する点は共通する。
...
本研究でも、前記研究同様、スタッフの判断知識を得るためにスタッフごとの回答をAHP重みとFuzzyルールへ変換するアプローチをとる。

## 2.3 Position of This Study

既存研究では、空間的位置を用いた転倒リスク、context-aware risk assessment、患者の連続リスクと優先化、専門家判断のパラメータ化がそれぞれ検討されている。一方、病棟共有空間において、患者文脈・空間的文脈・スタッフごとの判断知識を同一モデルで扱い、複数患者の時系列危険度と見守り優先順位を評価する研究は、確認した範囲で限定的である。①空間的文脈の考慮とスタッフの判断知識の反映を同時に実現すると、②特徴量の種類が膨大になることに加え、特徴量同士が相互作用を生じ、総合危険度を左右する。このため、特徴量の組合せごとにスタッフの判断知識を取得することは困難であり、①②の両立は容易でないと考えられる。
本研究は、Fuzzy推論とAHPを組み合わせた階層構造により、多種かつ相互作用を含む特徴量をスタッフの判断知識を用いて評価する方法を提案する。
...
*Draft note: source slide p.9. Final figureでは同一profileの2条件をoverlayし、RQ1の比較が直接読める形式へ再設計する。*➡P4のみ採用する方針。

- 患者文脈のみ：空間的文脈の違いによる差がなく、全患者の立ち上がりに対して同様に反応する。
- 空間的文脈の考慮あり（Proposed）：見守りあり・車椅子あり・点滴無＜＜見守り無・車椅子あり・点滴なし＜見守り無・車椅子あり・点滴あり　の順に危険度が大きくなっており、見守りが無いことを上位としつつ、さらに立ち上がりを1位とする結果が得られている。
- 総じて、空間的文脈を考慮した見守りとすることで、患者文脈のみでは区別できない状況の違いを反映した危険度評価が可能となることが確認できた。
...
シナリオ1:2x2の患者配置で、正面から4人を見渡す位置にスタッフが配置されている。スタッフから見て奥の2人との距離が、手前の2人よりも遠いため、奥の2人の空間的文脈における危険度は高い状況。ここで、スタッフから見て左側の2人が立ち上がった時の順位を、複数のスタッフの判断知識に基づくモデルの評価結果間で比較する。
（シナリオの図を入れる）

シナリオ2:2x2の患者配置で、4人の中央に、C,Dの2名の患者のみを見渡せ位置にスタッフが配置されている。スタッフからみてA,Bが他の2人よりも見えにくいため、この2人の空間的文脈における危険度は高い状況。ここで、スタッフから見て左側の2人が車椅子に座り点滴ポールの近くにいると設定する。この順位を、複数のスタッフの判断知識に基づくモデルの評価結果間で比較する。
（シナリオの図を入れる）


#### 結果

シナリオ1では、空間的文脈を重視するスタッフでは死角の人物のほうが優先となる評価結果が出た一方、患者文脈を重視するスタッフでは立ち上がったスタッフの方が優先される結果となった。
(波形を表示)

シナリオ2では、見守りの有無を周囲の物体よりも重視するスタッフでは死角にある患者を優先する評価結果が出た一方、周囲の物体を重視するスタッフでは死角にある患者よりも、車椅子に座り点滴ポールの近くにいる患者の方が優先される結果となった。
(波形を表示)
...
実際の病棟でのデータに基づいて危険度評価を行い、代表するスタッフ2名の判断知識に基づいた評価結果の比較を行う。また、全回答者（n=10）について、アンケートから得た判断指標と危険度出力の関係を分析する。
...
患者文脈を重視するP8のスタッフでは立ち上がった患者(00021)が最優先となった一方、空間的文脈を重視するP6のスタッフではスタッフから離れた位置にあって車椅子に座り、車椅子を動かそうとしている患者(00006)が最優先となった。
...
図中左（C_iが近いほう。つまり患者文脈を重視する）では00021が高い危険度評価をされる傾向にあり、図中右に行くにしたがって00021の危険度が低くなる傾向が分かる。
